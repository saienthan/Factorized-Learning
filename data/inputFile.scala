...
//morpheus
//w+alpha*(t5.reduce((acc,v)=>acc+v))
/*
val t1 = t.map(x => 1 + exp(x.dot(w)))
val t2 = y.zip(t1)
val matProd = t2.map(x=> x._1 / x._2)
*/
/*
val t1 = t.map((x) => x._2.dot(w))
val t2 = y.zip(1+exp(t1))
val t3 = t2.map((a)=>a._2._1/a._2._2)
val t4 = t3.zip(t)
val t5 = t4.map((x)=>x._2._1*x._2._2)
val t6 = t5.reduce((acc, v) => acc+v)
*/
/*
*/
/*
